...
int main(int argc, char** argv){
  ...
  /*
    ...
    a - nome do arquivo em que serao salvas as informacoes finais
    v - liga visualizacao
  */
  while( (c = getopt(argc, argv, "vL:A:j:s:C:q:t:T:c:f:P:r:e:a:") ) != -1 ){
    switch(c){
    case 'L':
      M = atoi(optarg);
      break;
  ...
  for(unsigned int k = 0; k < joanas.size(); k++){
    fprintf(file,"Joaninha %d:\n\tPosicao: [%d][%d]\tTemperatura:%lf\n",
	    k, joanas[k]->pos_i, joanas[k]->pos_j,
	    temp_hex(matriz, N, M, joanas[k]->pos_i, joanas[k]->pos_j, TEMP_FONTE));
  }
